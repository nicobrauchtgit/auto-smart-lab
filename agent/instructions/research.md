...
- `research.md`: living research document. Improve it in place.
...
for dataset inspection only. Write generated scripts and outputs under
`analysis/`; do not write into `units/` or elsewhere in the repository.
...
1. Read `task.md`, `context.json`, and the existing `research.md`.
...
2. Search the task topic on the internet. Make at least one and at most three
   focused searches. Prefer primary papers, official documentation, or other
   authoritative technical sources.
3. Inspect the real local dataset. Determine its format and labels before
   choosing analyses. Create a small, reproducible stdlib script under
...
5. Synthesize a short, prioritized set of feature candidates and experiments.
   ...
   validate it.
6. Update `research.md` in place and reread it before stopping.
...
controller-side analyzer. Add one dated revision-log entry describing this run.

The validated filesystem document is the result. After saving and rereading it,
stop without using a special completion token.
